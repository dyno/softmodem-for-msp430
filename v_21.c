...
void HardwareInit(void)
{
    ...
    WDTCTL = WDTPW + WDTHOLD;             // stop WDT, "10-8 Watchdog Timer"

    for (i = 0xFFFF; i > 0; i--);         // Delay for LFXT1 to settle "4.1 Basic Clock Module Introduction"

    // "LFXT => Low-frequency/high-frequency"
...
//"Silicon Data Access Arrangements (DAA), Phone Line Interface Telephone Network Interfaces"
...
#endif
#ifdef __MSP430__
interrupt(TIMERA0_VECTOR) TA_CCR0_ISR(void)
#endif
{
...
#endif
{
    ...
    case  2:
        ...
        if (Capture_Diff > RX_FREQ_CHN1_SPACE - CHN1_MARGIN
		&& Capture_Diff < RX_FREQ_CHN1_SPACE + CHN1_MARGIN) {
        ...
        } else if (Capture_Diff > RX_FREQ_CHN1_MARK - CHN1_MARGIN
		   && Capture_Diff < RX_FREQ_CHN1_MARK + CHN1_MARGIN) {
...
void Set_DCO(void)
{
    ...
    CCTL2 = 0;                              // Stop CCR2 function "Capture/Compare Control 2"
    TACTL = 0;                              // Stop Timer_A
    BCSCTL1 &= ~(DIVA1 + DIVA0);            // undivided ACLK "Basic Clock System Control 1"
}
